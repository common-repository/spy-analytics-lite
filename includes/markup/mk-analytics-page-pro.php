...
<div class="wrap">
    ...
    function prepare_items() {
		...
		global $wpdb;
		$table = $wpdb->prefix.$this->option['dbtable_name'];
        $query = "SELECT * FROM $table ORDER BY `session_start`";
        ...
        foreach ( $items as $item ) 
		{
			...
			//wp user
			$user_info = get_userdata( $usrData[2] );
			if(!$user_info)
				$user_name = "guest";
			else
				$user_name = '<a href="'. home_url().'/wp-admin/user-edit.php?user_id='.$user_info->ID.'" target="_blank">'.$user_info->user_login.'</a>';

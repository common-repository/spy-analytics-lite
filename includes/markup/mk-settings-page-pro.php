					...
					<fieldset>
						...
						<p>
							<label for="opt_record_all_2">
								<input name="opt_record_all" id="opt_record_all_2" type="radio" value="no" class="tog" <?php print(($defaults["opt_record_all"])?'':'checked="checked"'); ?>>
								Special page or/and post</label>
						</p>
						<ul>
							<li>
								<label for="opt_record_all_2">
									<select name="opt_record_special[]" multiple="multiple" size="10" style="min-height: 8em;">
										...
										<optgroup label="Posts">
											<?php
											$pages = get_posts( array( 'numberposts' => 99999 ));
											foreach ($pages as $page) {
												$checked = (in_array($page -> ID, $defaults['opt_record_special']))?'selected="selected"':'';
												$option = '<option value="' . $page -> ID . '" '.$checked.' >';
												$option .= $page -> post_title;
